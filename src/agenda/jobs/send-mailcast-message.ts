...
export const defineSendMailcastMessageJob = (agenda: Agenda, fastify: FastifyInstance) => {
  agenda.define(
    'send-mailcast-message',
    async (job: Job<MailcastSendMessageInput & { taskId?: string }>) => {
      ...
      try {
        ...
        // The WA Agents will handle that when it actually sends the message

        fastify.log.info(
          { subject, taskId, agentId },
          '[Agenda] Mailcast message published to JetStream'
        );
      } catch (err) {
